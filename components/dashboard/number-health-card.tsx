...
export function NumberHealthCard() {
  ...
  return (
    <div className="relative flex h-full flex-col overflow-hidden rounded-[1.5rem] border border-white/[0.08] bg-[linear-gradient(180deg,rgba(255,255,255,0.055),rgba(255,255,255,0.02))] shadow-[0_20px_70px_rgba(0,0,0,0.34)] backdrop-blur-xl">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_18%_0%,rgba(52,211,153,0.12),transparent_34%)]" aria-hidden />
      ...
      {loading ? (
        <div className="relative flex-1 space-y-2 px-5 pb-5">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-14 animate-pulse rounded-xl border border-white/[0.04] bg-white/[0.02]" />
          ))}
        </div>
      ) : active.length === 0 ? (
        <div className="relative flex flex-1 flex-col items-center justify-center gap-3 px-5 py-10 text-center">
        ...
        <>
          <div className="relative grid grid-cols-3 gap-2 px-5 pb-4">
            {[
              ...
              <div
                key={stat.label}
                className="rounded-xl border border-white/[0.06] bg-white/[0.035] px-3 py-2.5 text-center"
              >
          ...
          {needsCheck > 0 && (
            <p className="relative px-5 pb-3 text-[11px] text-slate-500">
              {needsCheck} line{needsCheck === 1 ? '' : 's'} ready to verify.
            </p>
          )}

          <div className="relative flex-1 space-y-1.5 overflow-y-auto px-5 pb-5">
            {active.slice(0, 6).map((num, i) => {
              ...
              return (
                <motion.div
                  ...
                  className="flex items-center gap-3 rounded-xl border border-white/[0.05] bg-white/[0.025] px-3 py-2.5 transition-colors hover:border-white/[0.08] hover:bg-white/[0.045]"
                >
          ...
          {active.length > 6 && (
            <Link
              href="/numbers"
              className="relative block border-t border-white/[0.04] px-5 py-3 text-center text-xs text-white/40 transition-colors hover:text-white/60"
            >
