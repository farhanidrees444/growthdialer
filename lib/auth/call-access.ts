...
export async function findCall(
  ...
  let query = supabase
    .from('calls')
    .select('id, user_id, workspace_id, telnyx_call_id, telnyx_session_id, telnyx_webrtc_leg_id, from_number, to_number, lead_id, disposition, direction, status, answered_at');
